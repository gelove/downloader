...
function ffmpegProgress(
  ...
  // 命令执行结束时触发
  cmd.on("close", (_data) => {
    log.debug("ffmpegProgress close");
  ...
  cmd.stderr.on("data", (line) => {
    ...
    if (line.includes("out_time_us=")) {
      ...
      // 计算得到进度比例
      const percentage = _duration / duration;
      // log.debug("percentage =>", percentage, typeof percentage);
      // 转换为以%号显示的数值
      appWindow.emit(action, {
        id,
 ...
 * @returns 命令对象
 */
export async function aiToolCmd(dirname: string) {
  let model_path = await resolve(await appConfigDir(), "models", "large-v3");
  // return Command.create("bin/ai_tool", [dirname, "-m", model_path]);
  return Command.sidecar("bin/ai_tool", [dirname, "-m", model_path]);
}

// 执行 python 可执行文件 ai_tool or ai_tool.exe
// 利用 AI 处理视频实现语言转换和嘴唇同步
export async function execAiTool(dirname: string) {
  const command = await aiToolCmd(dirname);
  const appWindow = getCurrentWebviewWindow();
 ...
 * @returns 命令行对象
 */
export async function videoToLandscapeCmd(
  src: string,
  dest: string,
  width: number = 1280,
  height: number = 720,
 ...
 */
export async function videoToPortraitCmd(
  src: string,
  dest: string,
  width: number = 720,
  height: number = 1280,
...
export async function videoChangeOrientation(
  ...
  switch (orientation) {
    case VideoOrientation.Landscape:
      cmd = await videoToLandscapeCmd(filepath, dest);
      break;
    case VideoOrientation.Portrait:
      cmd = await videoToPortraitCmd(filepath, dest);
      break;
    default:
      break;
  }
 ...
 */
export async function extractAudioCmd(
  src: string,
  ext: string = "aac",
  bitRate?: string,
  sampleRate?: string,
 ...
 */
export async function extractVideoCmd(
  src: string,
  dest?: string,
  start?: string,
  duration?: string,
 ...
 * @param dest
 */
export async function splitVideoAndAudioCmd(src: string, dest: string, audioExt: string = "aac") {
  // if (!dest) {
  //   dest = src;
  // }
  // const filepath = await invoke<string>("get_output_filepath", { dest });
...
export async function splitVideoAndAudio(
  ...
  let cmd = await splitVideoAndAudioCmd(filepath, dest, audioExt);
  if (!cmd) {
 ...
 * 添加合并音频
 */
export async function mergeAudioCmd(video: string, audio: string, dest: string, time?: number) {
  // 视频变小了 似乎被压缩了
...
export async function mergeAudio(
  ...
  dest = await invoke<string>("get_output_filepath", { dest });
  let cmd = await mergeAudioCmd(filepath, audio, dest, time);
  if (!cmd) return;
 ...
 * @returns 命令行对象
 */
export async function videoTranscodeCmd(src: string, dest: string, resolution: string) {
  if (!src || !dest || !resolution) return;
  // 获取输出路径（需要检测重复文件名）
  const filepath = await invoke<string>("get_output_filepath", { dest });
  // 构建视频转换命令
  const cmdArr = ["-i"];
 ...
 * @returns 命令行对象
 */
export async function audioTranscodeCmd(src: string, dest: string, bitRate: string) {
  if (!src || !dest || !bitRate) return;
  const cmdArr = ["-i"];
  cmdArr.push(src);
  cmdArr.push("-b:v");
  cmdArr.push(bitRate);
...
export async function transcode(
  ...
  //   data.convertDpiOrBitRateIndex == 0 ? data.dpiOrBitRate : data.convertDpiOrBitRate;
  ...
  switch (data.kind) {
    case Media.Audio:
      cmd = await audioTranscodeCmd(filepath, dest, quality);
      break;
    case Media.Video:
      cmd = await videoTranscodeCmd(filepath, dest, quality);
      break;
  }
