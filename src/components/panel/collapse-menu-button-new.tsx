...
import { isSubMenuActive } from "@/lib/route";
...
export function CollapseMenuButton({
  ...
  const submenuActive = submenus.some((submenu) => submenu.active);
  const [isCollapsed, setIsCollapsed] = useState<boolean>(submenuActive);
      ...
      <Collapsible open={isCollapsed} onOpenChange={setIsCollapsed} className="w-full">
        <CollapsibleTrigger className="mb-1 [&[data-state=open]>div>div>svg]:rotate-180" asChild>
          <Button
            ...
            className="h-10 w-full justify-start border-0">
        ...
        <CollapsibleContent className="overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down">
          {submenus.map(({ href, label }, index) => {
            const isActive = isSubMenuActive(pathname, href);
            return (
              <Button
                key={index}
        ...
        <Tooltip delayDuration={100}>
          <TooltipTrigger asChild>
            <DropdownMenuTrigger asChild>
              <Button
                variant={active ? "secondary" : "ghost"}
                className="mb-1 h-10 w-full justify-start border-0">
                <div className="flex w-full items-center justify-between">
                  <div className="flex items-center">
                    <span>
                      <Icon size={18} />
